...
function updateRGBCounts() {
	document.getElementById("rint").value =
		document.getElementById("redIntensity").value;
	document.getElementById("gint").value =
		document.getElementById("greenIntensity").value;
	document.getElementById("bint").value =
		document.getElementById("blueIntensity").value;
	document.getElementById("huee").value =
		document.getElementById("hue").value;
	document.getElementById("satt").value =
...
function initUI() {
	document.querySelector("#char").oninput = () => {
		settings.char = document.getElementById("char").value;
		loadNewImage(settings.last_source);
	};

	document.querySelector("#redIntensity").onchange = () => {
		updateRGBCounts();
		loadNewImage(settings.last_source);
	};
	document.querySelector("#greenIntensity").onchange = () => {
		updateRGBCounts();
		loadNewImage(settings.last_source);
	};
	document.querySelector("#blueIntensity").onchange = () => {
		updateRGBCounts();
		loadNewImage(settings.last_source);
	};
	document.querySelector("#hue").onchange = () => {
		updateRGBCounts();
		loadNewImage(settings.last_source);
	};
	document.querySelector("#saturation").onchange = () => {
		updateRGBCounts();
		loadNewImage(settings.last_source);
	};
	document.querySelector("#value").onchange = () => {
		updateRGBCounts();
		loadNewImage(settings.last_source);
	};
	document.querySelector("#threshold").onchange = () => {
		updateRGBCounts();
		loadNewImage(settings.last_source);
	};
	...
	setUIElement("#boxes", settings.boxes).onchange = (e) => {
		settings.boxes = e.target.checked;
		loadNewImage(settings.last_source);
	};
	setUIElement("#minecraft", settings.minecraft).onchange = (e) => {
		settings.minecraft = e.target.checked;
		loadNewImage(settings.last_source);
	};
	setUIElement("#autocopy", settings.autocopy).onchange = (e) => {
		settings.autocopy = e.target.checked;
		loadNewImage(settings.last_source);
	};

	setUIElement("#I4", settings.I4).onchange = (e) => {
		settings.I4 = e.target.checked;
		loadNewImage(settings.last_source);
	};
	...
	setUIElement("#huee", settings.huee).onchange = (e) => {
		settings.huee = e.target.value;
		document.getElementById("hue").value = settings.huee;
		loadNewImage(settings.last_source);
	};
	...
	setUIElement("#vall", settings.vall).onchange = (e) => {
		settings.vall = e.target.value;
		document.getElementById("value").value = settings.vall;
		loadNewImage(settings.last_source);
	};
	...
	document.querySelector("#autoScale").onclick = (e) => {
		...
		if (
			document.querySelector("#charcount").innerText <= 4200 ||
			document.querySelector("#charcount").innerText >= 4500
		) {
			// base to estimate
	...
	document.querySelector("#hReset").onclick = (e) => {
		document.getElementById("huee").value = 1;
		document.getElementById("hue").value = 1;
		loadNewImage(settings.last_source);
	};
	...
	document.querySelector("#vReset").onclick = (e) => {
		document.getElementById("vall").value = 1;
